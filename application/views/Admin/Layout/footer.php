...
<footer class="main-footer">
	<strong>TOKO JAYA PLASTIK</strong> | Repost by <a href='https://' title='Arkan n Vian' target='_blank'>ArkanDarvesh and FaizHaq</a>
...
<script>
	...
	foreach ($analisis as $key => $value) {
		$member[] = $value->nm_member;
		$tt[] = $value->qty_member;
	}
	?>
	var ctx = document.getElementById('grafik_pelanggan_member');
	var grafik = new Chart(ctx, {
		type: 'pie',
		data: {
			labels: <?= json_encode($member) ?>,
			datasets: [{
				label: 'Grafik Analisis Member Pelanggan',
				data: <?= json_encode($tt) ?>,
				backgroundColor: [
					'rgba(255, 99, 132, 0.80)',
					'rgba(54, 162, 235, 0.80)',
